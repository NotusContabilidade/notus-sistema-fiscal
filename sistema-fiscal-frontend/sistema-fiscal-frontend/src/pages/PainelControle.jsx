import React, { useState, useEffect, useMemo, useCallback } from "react"; // <-- CORREÇÃO APLICADA AQUI
  ...
  return (
    <div className="workflow-card">
      <div className="card-header">
        <span className="card-tipo-icon">{item.tipo === 'TAREFA' ? <CheckSquare size={16} /> : <FileText size={16} />}</span>
...
export default function PainelControle() {
  ...
  const handleCriarTarefa = async (e) => {
    ...
    try {
      ...
      fetchWorkflowItems(); // Atualiza o painel com a nova tarefa
    } catch(err) {
          ...
          <>
            <div className="workflow-coluna">
              ...
              <div className="coluna-content">
                {colunas.pendentes.map(item => <WorkflowCard key={`${item.tipo}-${item.id}`} item={item} />)}
            ...
            <div className="workflow-coluna">
              <h3>Concluídas</h3>
              <div className="coluna-content">
                {colunas.concluidos.map(item => <WorkflowCard key={`${item.tipo}-${item.id}`} item={item} />)}
      ...
      <Modal isOpen={isModalOpen} onRequestClose={() => setIsModalOpen(false)} style={{ content: { maxWidth: '600px', margin: 'auto', height: 'fit-content' } }}>
        ...
        <form onSubmit={handleCriarTarefa} className="modal-body form-nova-demanda">
          <div className="form-group">
            <label>Cliente*</label>
            <select name="clienteId" value={novaTarefa.clienteId} onChange={handleInputChange} required>
          ...
          <div className="form-group">
            ...
            <input type="text" name="titulo" value={novaTarefa.titulo} onChange={handleInputChange} placeholder="Ex: Enviar DAS Ref. 08/2025" required />
          </div>
          <div className="form-group">
            <label>Categoria</label>
            <select name="categoria" value={novaTarefa.categoria} onChange={handleInputChange}>
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input type="text" name="responsavel" value={novaTarefa.responsavel} onChange={handleInputChange} placeholder="Nome do membro da equipe" />
            </div>
            <div className="form-group">
              <label>Prazo Final</label>
              <input type="date" name="prazo" value={novaTarefa.prazo} onChange={handleInputChange} />
            </div>
          </div>
          <div className="form-group">
            <label>Descrição / Detalhes</label>
            <textarea name="descricao" value={novaTarefa.descricao} onChange={handleInputChange} rows="3"></textarea>
          </div>
          <div className="modal-actions">
            <button type="button" className="btn-secundario" onClick={() => setIsModalOpen(false)}>Cancelar</button>
            <button type="submit" className="btn-primario" disabled={isLoading}>{isLoading ? <Spinner /> : 'Criar Demanda'}</button>
          </div>
        </form>
      </Modal>
    </div>
